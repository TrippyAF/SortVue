...
const partition = async function (l, r) {
	...
	for (let j = l; j < r; ++j) {
		if (bars[j] < pivot) {
			...
			barsEl.children[i].children[0].style.backgroundColor = barDefaultColor;
...
const quickS = async function (l, r) {
	...
	await quickS(l, pivotIndex - 1);
	await quickS(pivotIndex + 1, r);
};
...
const quickSort = async function () {
	...
	await quickS(0, Number(size - 1)).then(() => doGreen(size));
